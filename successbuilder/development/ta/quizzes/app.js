...
const quizCatalog = [
    {
        ...
        description:
            "Washington State electrical law covering licensing, inspections, and contractor requirements.",
        file: "data/rcw-19-28.json",
        questionCount: null,
    },
    {
        id: "ta_pq3",
        title: "Telco Admin Practice Quiz 3",
        description:
            "NM cable, grounding electrodes, boxes/enclosures, and related theory.",
        file: "data/ta_pq3.json",
        questionCount: null,
    },
    {
        id: "telco-general-theory-01",
        title: "Telecom Admin — General Trade Knowledge & Theory",
        description:
            "General trade knowledge and communications theory practice questions for telecom administrator prep.",
        file: "data/telco-general-theory-01.json",
        questionCount: null,
    },
];
...
async function renderCatalog() {
    ...
    for (const quiz of quizCatalog) {
      ...
      <div class="catalog-card-meta">${count} question${count === 1 ? "" : "s"
            }</div>
      <p style="font-size:var(--text-xs);color:var(--color-text-muted);max-width:36ch;margin-bottom:var(--space-4);">${escHtml(quiz.description)
            }</p>
...
async function loadQuiz(quiz) {
    ...
    /** @type {HTMLInputElement | null} */
      const shuffleQ = /** @type { q"));
    /** @type {HTMLInputElement | null} */
      const shuffleC = /** @type { c"));
...
function renderQuestion() {
    ...
    choices.forEach((choice, i) => {
        ...
        li.innerHTML = `
      <span class="choice-letter" aria-hidden="true">${letters[i] || i + 1
            }</span>
...
function showResults() {
    ...
    /** @type {HTMLElement} */
      const scorePct = /** @type { ));
    /** @type {HTMLElement} */
      const resultsHeading = /** @type { ));
    /** @type {HTMLElement} */
      const resultsSub = /** @type { ));
    ...
    reviewBtn.addEventListener("click", () => {
        ...
        state.results.forEach((r, i) => {
            ...
            const choicesHtml = r.question.choices.map((c) => {
                ...
                return `<div class="review-answer ${cls}">${badge} <span>${escHtml(c)
                    }</span></div>`;
            }).join("");
        ...
        ${r.question.explanation
                    ...
                    escHtml(r.question.explanation) + "</p>"
                    : ""
                }
      `;
